...
class Future(object):
    def __init__(self, task_id, task_size, future_cache_ref, task_name):
        self._id = task_id
...
class _BaseStreamer(object):
    ...
    def _send_request(self, task_id, request_id, model_input, task_name):
        raise NotImplementedError

    def _recv_response(self, timeout=TIMEOUT):
        raise NotImplementedError

    def _input(self, batch: List, task_name: str) -> int:
        """
        ...
        future = Future(task_id, len(batch), weakref.ref(self._future_cache), task_name)
        self._future_cache[task_id] = future

        for model_input in batch:
            self._send_request(task_id, request_id, model_input, task_name)
            request_id += 1
    ...
    def _output(self, task_id: int, task_name: str) -> List:
        future = self._future_cache[task_id]
        batch_result = future.result(self._worker_timeout)
        return batch_result

    def submit(self, batch, task_name):
        task_id = self._input(batch, task_name)
        future = self._future_cache[task_id]
        return future

    def predict(self, batch, task_name):
        task_id = self._input(batch, task_name)
        ret = self._output(task_id, task_name)
        assert len(batch) == len(ret), "input batch size {} and output batch size {} must be equal.".format(len(batch),
                                                                                                            len(ret))
        return ret
...
class _BaseStreamWorker(object):
    def __init__(self, base_model, predict_function, batch_size, max_latency, *args, **kwargs):
        super().__init__()
        ...
        self.task_n = len(self._base_model.lora2path)
    ...
    def _run_once(self):
        ...
        # all_tasks = list(self._base_model.lora2path.keys())
        task2batch = defaultdict(list)
        for i in range(self._batch_size * self.task_n):
            try:
                ...
                task_name = item[-1]
                task2batch[task_name].append(item)
                if len(task2batch[task_name]) > self._batch_size:
                    break

        total_batch_size = 0
        for task_name, batch in task2batch.items():
            if self._base_model.get_cur_lora() != task_name:
                self._base_model.set_lora(task_name)
            total_batch_size += len(batch)

            model_inputs = [i[3] for i in batch]
            model_outputs = self.model_predict(model_inputs)
            print('--' * 50)
            print(f'task name: {task_name}, batch size: {len(model_outputs)}')

            # publish results to redis
            for i, item in enumerate(batch):
                client_id, task_id, request_id, _, _ = item
...
#
#     def _send_request(self, task_id, request_id, model_input, task_name):
#         self._input_queue.put((0, task_id, request_id, model_input, task_name))
#
#     def _recv_response(self, timeout=TIMEOUT):
#         try:
...
class Streamer(_BaseStreamer):
    def __init__(self, base_model, predict_function_or_model, batch_size, max_latency=0.1, worker_num=1,
        ...
        self._worker = StreamWorker(base_model, predict_function_or_model, batch_size, max_latency,
                                    self._input_queue, self._output_queue,
...
class StreamWorker(_BaseStreamWorker):
    def __init__(self, base_model, predict_function_or_model, batch_size, max_latency, request_queue, response_queue,
                 model_init_args, model_init_kwargs, *args, **kwargs):
        super().__init__(base_model, predict_function_or_model, batch_size, max_latency, *args, **kwargs)
        self._request_queue = request_queue
        self._response_queue = response_queue
...
#
#     def _send_request(self, task_id, request_id, model_input, task_name):
#         self._redis.send_request(task_id, request_id, model_input, task_name)
#
#     def _recv_response(self, timeout=TIMEOUT):
